...
void SortList(Student **head) {  
    ...
    Student *sorted = NULL; // 创建一个新的有序链表  
    Student *current = *head;  
    Student *next;  
  
    // 遍历原链表，每次取出一个节点插入到有序链表中  
    while (current != NULL) {  
        next = current->next; // 保存下一个节点  
        Student *prev = NULL;  
        Student *currentPos = sorted;  
  
        // 找到current应该插入的位置  
        while (currentPos != NULL && currentPos->score > current->score) {  
            prev = currentPos;  
        ...
        if (prev == NULL) {  
            sorted = current; // 插入到开头  
        } else {  
            prev->next = current; // 插入到中间或末尾  
        }  
  
        current = next; // 继续遍历原链表  
    }  
