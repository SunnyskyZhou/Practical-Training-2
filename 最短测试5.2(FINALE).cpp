...
Edge* createEdge(int dest, int weight)
 {  
    ...
    if (!edge)  // 检查内存分配是否成功（检验权重edge是否为假值） 
    {  
...
{  
    ...
    // 添加反向边（也可在主函数main里算两次算法来使得顺序和逆序都可以计算，反过来计算，不过这里直接省略） 
    edge = createEdge(src, weight);  // 创建反向边-原目的地指向出发地 
...
{
    int min = INT_MAX, min_index = -1;  // 初始化最小值为-1 
...
// 打印从源点到目标点的最短路径
void printPath(int parent[], int j)   //举例: 令j是源节点
{
...
{  
    ...
    printf("[地址编号表] \n");
    printf("0-乌鲁木齐, 1-兰州,2-西宁,3-呼和浩特,4-北京,5-天津  \n ");
	printf("6-沈阳,7-长春,8-哈尔滨,9-大连,10-西安,11-郑州  \n"); 
	printf("12-徐州,13-拉萨,14-成都,15-武汉,16-上海,17-昆明  \n");
    printf("18-贵阳,19-株洲,20-南昌,21-福州,22-柳州,23-南宁,24-广州,25-深圳\n");
	printf("\n");
    printf("请输入出发地的编号: ");
